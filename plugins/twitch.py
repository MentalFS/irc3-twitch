...
class Twitch:
	...
	@irc3.event(r'@(\S+;)?room-id=(?P<user>\d+)(;\S+)? :\S+ ROOMSTATE #(?P<channelname>\S+)( :.*)?', iotype='in')
	...
	@irc3.event(r'(@\S+ )?PART #(?P<channelname>\S+)( :.*)?', iotype='out')
	...
	@irc3.event(r'(@\S+ )?NOTICE (?P<target>\S+) :(?P<data>.*)', iotype='out')
	...
	@irc3.event(r'(@\S+ )?PRIVMSG (?P<user>[^#]\S+) :(?P<data>.*)', iotype='out')
